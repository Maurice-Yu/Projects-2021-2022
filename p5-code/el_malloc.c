...
el_blockhead_t *el_block_below(el_blockhead_t *block){
  if(PTR_MINUS_BYTES(block,sizeof(el_blockfoot_t)) < (void*) el_ctl->heap_start){
  return NULL;
}
...
el_blockhead_t *el_find_first_avail(size_t size){
	...
	el_blockhead_t *spot= el_ctl->avail->beg;
...
el_blockhead_t *el_split_block(el_blockhead_t *block, size_t new_size){
  ...
  {
  el_blockfoot_t *foot= PTR_PLUS_BYTES(block,new_size+sizeof(el_blockhead_t));
  el_blockhead_t *head= PTR_PLUS_BYTES(foot,sizeof(el_blockfoot_t));
  foot->size=(new_size);
  head->size=(block->size-new_size-EL_BLOCK_OVERHEAD);
  block->size=foot->size;
  el_get_footer(head)->size=head->size;
  return head;
  }
  return NULL;
}
...
void *el_malloc(size_t nbytes){
  ...
  el_blockhead_t *point = el_find_first_avail(nbytes);
  void *ptr =point;
  if (point->size>=nbytes+EL_BLOCK_OVERHEAD)
  {
   el_remove_block(el_ctl->avail,point);
   point = el_split_block(point,nbytes);
   point->state=EL_AVAILABLE;
   el_add_block_front(el_ctl->avail,point);
   point = el_block_below(point);
   ptr = point;
  }
  else
  {
  el_remove_block(el_ctl->avail,point);
  point->state=EL_USED;
  el_add_block_front(el_ctl->used,point);
  ptr=PTR_PLUS_BYTES(ptr,sizeof(el_blockhead_t));
...
void el_merge_block_with_above(el_blockhead_t *lower){
	...
	{
	if(lower->state== EL_AVAILABLE && upper->state== EL_AVAILABLE)
 	{
 	 el_remove_block(el_ctl->avail,lower);
 	 lower->size+=(upper->size+EL_BLOCK_OVERHEAD);
...
void el_free(void *ptr){
	el_blockhead_t *point = PTR_MINUS_BYTES(ptr,sizeof(el_blockhead_t));
	if(point->state==EL_USED)
	{
	point->state=EL_AVAILABLE;
	el_remove_block(el_ctl->used,point);
	el_add_block_front(el_ctl->avail,point);

	if(1)
	{
	el_merge_block_with_above(point);
	}
	
	if(1)
	{
	point=el_block_below(point);
	if((void *)point>=(void *)el_ctl->heap_start)
	{
	el_merge_block_with_above(point);
	}
	}
...
int el_append_pages_to_heap(int npages){
	if((mmap(el_ctl->heap_end,
         EL_PAGE_BYTES*npages,
         PROT_READ | PROT_WRITE,
         MAP_PRIVATE | MAP_ANONYMOUS,
         -1, 0))==MAP_FAILED)
         {
...
el_blockhead_t *new= el_ctl->heap_end;
el_ctl->heap_end = PTR_PLUS_BYTES(el_ctl->heap_end,EL_PAGE_BYTES*npages);
el_ctl->heap_bytes+= EL_PAGE_BYTES*npages;
new->state=EL_AVAILABLE;
el_blockfoot_t *foot= PTR_PLUS_BYTES(new,EL_PAGE_BYTES*npages-sizeof(el_blockfoot_t));
foot->size=((size_t) EL_PAGE_BYTES*npages)-EL_BLOCK_OVERHEAD;
new->size=(foot->size);
el_add_block_front(el_ctl->avail,new);
el_merge_block_with_above(el_block_below(new));
return 0;
}
